...
class hickory_homepage_widget extends WP_Widget {
	...
	function hickory_homepage_widget() {
		...
		/* Create the widget. */
		parent::__construct( 'hickory_homepage_widget', __('Hickory: Homepage Post Widget', 'hickory_homepage_widget'), $widget_ops, $control_ops );
	}
				...
				<div class="item">
					...
					<?php if($layout == 'list') : ?>
						...
						<?php if(hick_option('hick_homepage_sidebar') == 0 && is_home() || is_page_template('page_fullwidth.php')) : ?>
							...
							<?php endif; ?>
						<?php elseif(is_page() || hick_option('hick_homepage_sidebar') == 1 || is_category()) : ?>
							<?php if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail())  ) : ?>
