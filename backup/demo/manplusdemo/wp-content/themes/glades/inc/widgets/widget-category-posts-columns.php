...
class Glades_Category_Posts_Columns_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'classname' => 'glades_category_posts_columns', 

			'description' => esc_html__( 'Displays your posts from two selected categories. Please use this widget ONLY in the Magazine Homepage widget area.', 'glades' ),

			'customize_selective_refresh' => true,

		);

		parent::__construct('glades_category_posts_columns', sprintf( esc_html__( 'Category Posts: 2 Columns (%s)', 'glades' ), wp_get_theme()->Name ), $widget_ops);
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete('widget_glades_category_posts_columns', 'widget');
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'widget_glades_category_posts_columns', 'widget' );
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_glades_category_posts_columns', $cache, 'widget' );
	...
	function display_category_posts( $settings, $category_id ) {
		...
		add_filter('excerpt_length', 'glades_category_posts_small_excerpt');
				...
				?>
					...
					<article id="post-<?php the_ID(); ?>" <?php post_class('large-post clearfix'); ?>>
						...
						<div class="entry">
							...
							<a href="<?php esc_url(the_permalink()) ?>" class="more-link"><?php esc_html_e( 'Xem thêm &raquo;', 'glades' ); ?></a>
		...
		remove_filter('excerpt_length', 'glades_category_posts_small_excerpt');
	...
	function display_category_title( $args, $settings, $category_id, $category_title ) {
		...
		if( !empty( $widget_title ) ) :
			...
			if( $category_id == 0 ) :
				...
				$link_title = sprintf( esc_html__( 'View all posts from category %s', 'glades' ), get_cat_name( $category_id ) );
	...
	function form( $instance ) {
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('category_one_title'); ?>"><?php esc_html_e( 'Left Category Title:', 'glades' ); ?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('category_one'); ?>"><?php esc_html_e( 'Left Category:', 'glades' ); ?></label><br/>
			...
			<label for="<?php echo $this->get_field_id('category_two_title'); ?>"><?php esc_html_e( 'Right Category Title:', 'glades' ); ?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('category_two'); ?>"><?php esc_html_e( 'Right Category:', 'glades' ); ?></label><br/>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id('highlight_post'); ?>">
				...
				<?php esc_html_e( 'Highlight first post (big image + excerpt)', 'glades' ); ?>
